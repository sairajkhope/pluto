...
class GetUpOpenPolicy(BasePolicy):
    def __init__(
        self, name: str, robot: Robot, init_motor_pos: npt.NDArray[np.float32]
    ):
        """Initializes the GetUpOpenPolicy with specific parameters."""
        super().__init__(name, robot, init_motor_pos)

        robot_suffix = "_2xc" if "2xc" in robot.name else "_2xm"
        motion_file_path = os.path.join("motion", f"get_up_chair{robot_suffix}.lz4")
