...
from toddlerbot.reference.crawl_down_stairs_ref import CrawlDownStairsReference
from toddlerbot.sim import BaseSim, Obs
from toddlerbot.sim.robot import Robot
from toddlerbot.tools.joystick import Joystick


class CrawlDownStairsPolicy(MJXPolicy):
    """Crawl down stairs policy for the toddlerbot robot."""
    ...
    def __init__(
        ...
        """Initializes the CrawlDownStairsPolicy with specific parameters."""
        super().__init__(name, robot, init_motor_pos, path, joystick, fixed_command)

        self.motion_ref = CrawlDownStairsReference(robot, self.control_dt)
        state_ref = self.motion_ref.get_default_state()
        state_ref = self.motion_ref.get_state_ref(0.0, np.zeros(3), state_ref)
        self.default_action = state_ref["motor_pos"][self.action_mask]
        self.ref_motor_pos = state_ref["motor_pos"].copy()

        self.num_frames = self.motion_ref.full_episode_length
        self.count = 0
        self.is_done = False
