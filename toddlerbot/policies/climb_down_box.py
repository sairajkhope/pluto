...
from toddlerbot.reference.climb_down_box_ref import (
    ClimbDownBoxReference,
)
...
class ClimbDownBoxPolicy(MJXPolicy):
    def __init__(
        self,
        name: str,
        robot: Robot,
        ...
        """Initializes the ClimbDownBoxPolicy with specific parameters."""
        super().__init__(name, robot, init_motor_pos, path, joystick, fixed_command)

        self.motion_ref = ClimbDownBoxReference(robot, self.control_dt)
    ...
    def get_phase_signal(self, time_curr: float, init_idx: int = 0, num_frames=None):
        ...
        # phase = (
        #     (total_idx / num_frames) * 2 * np.pi
        #     if time_idx < num_frames
        #     else (num_frames / num_frames) * 2 * np.pi
        # )
