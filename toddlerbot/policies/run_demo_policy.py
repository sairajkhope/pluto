...
POLICY_CONFIGS = {
    ...
    "crawl": {
        "policy": "crawl_only_demo",
        "skill": "crawl_only_demo",
        "type": "transition",
    },
    "push_up": {
        "policy": "push_up_crawl",
        "skill": "push_up_crawl",
        "type": "transition",
    },
    "stand": {
...
def run_multiple_policy(
    ...
    def look_around():
        try:
            ...
            neck_yaw_sample_interval = 150
            neck_yaw_min_delta = 1.0  # Minimum delta threshold for neck movement
            neck_yaw_interpolation_rate = 0.02  # Rate of linear interpolation
            neck_yaw_direction_sign = 1  # Alternating direction sign

            obs = sim.get_observation()
            action = obs.motor_pos.copy()
            neck_yaw_target = action[0]  # Initialize target to current position

            while True:
                if step_count % neck_yaw_sample_interval == 0:
                    # Generate target with alternating direction
                    base_angle = np.random.uniform(neck_yaw_min_delta, 1.57)
                    new_neck_yaw_target = neck_yaw_direction_sign * base_angle
                    neck_yaw_direction_sign *= -1  # Reverse sign for next time

                    # Only update target if delta is large enough
                    if abs(new_neck_yaw_target - neck_yaw_target) >= neck_yaw_min_delta:
                        neck_yaw_target = new_neck_yaw_target

                # Linear interpolation towards target
                current_yaw = action[0]
                action[0] = current_yaw + neck_yaw_interpolation_rate * (
                    neck_yaw_target - current_yaw
                )
                ...
                if (
                    "real" in sim.name or args.vis == "view"
                ) and time_until_next_step > 0:
    ...
    print("Press CRTL+C to look at 90 degrees...")
    look_around()

    def look_at_fixed_target(neck_yaw_target):
        try:
            step_count = 0
            loop_start_time = time.monotonic()
            neck_yaw_interpolation_rate = 0.02  # Rate of linear interpolation
    ...
    print("Press CRTL+C to look at -90 degrees...")
    look_at_fixed_target(-np.pi / 2)

    print("Press CRTL+C to start...")
    look_at_fixed_target(np.pi / 2)
    ...
    try:
        ...
        while True:
            ...
            if (
                ...
                and "pull_up" not in previous_policy_key
            ):
                if abs(body_forward_vec[2]) > 0.5:
                    if (
                        ...
                        and "pull_up" not in current_policy_key
            ...
            if hasattr(current_policy, "is_done") and current_policy.is_done:
                if "get_off_cart" in current_policy_key:
                    ...
                    switch_state.switch_requested = True
                    switch_state.new_policy = "crawl"
                elif "crawl" in current_policy_key:
                    speak("My muscles are calling—let me do a few push-ups to warm up!")
                    switch_state.switch_requested = True
                    switch_state.new_policy = "push_up"
                elif "push_up" in current_policy_key:
                    speak("Phew! That was a nice stretch—now let's stand up tall!")
                    switch_state.switch_requested = True
                    switch_state.new_policy = "stand"
                elif "pull_up" in current_policy_key:
                    speak(
                        "Easy peasy! I could do this for hours before my battery runs out—but maybe not today hehe."
                    )
                    switch_state.switch_requested = True
                    switch_state.new_policy = "stand"
