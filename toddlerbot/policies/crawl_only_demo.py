...
class CrawlOnlyDemoPolicy(BasePolicy):
    def __init__(
        ...
        motion_file_path = os.path.join("motion", f"crawl_only_demo{robot_suffix}.lz4")
        data_dict = joblib.load(motion_file_path)
        base_action_arr = np.array(data_dict["action"], dtype=np.float32)
        self.ref_qpos = np.array(data_dict["qpos"], dtype=np.float32)[0]

        # Loop the entire motion 5 times
        n_loops = 2
        time_list = []
        action_list = []
        current_time = 0.0

        self.loop_start_index = int(0.5 / self.control_dt)
        for i in range(self.loop_start_index):
            time_list.append(current_time)
            action_list.append(base_action_arr[i])
            current_time += self.control_dt

        for _ in range(n_loops):
            for i in range(len(base_action_arr[self.loop_start_index :])):
                time_list.append(current_time)
                action_list.append(base_action_arr[self.loop_start_index + i])
                current_time += self.control_dt

        self.time_arr = np.array(time_list)
        self.action_arr = np.array(action_list, dtype=np.float32)
