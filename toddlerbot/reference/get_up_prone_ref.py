...
class GetUpProneReference(MotionReference):
    ...
    def __init__(self, robot, dt: float, fixed_base: bool = False):
        ...
        """
        super().__init__("get_up_prone", "keyframe", robot, dt, fixed_base)

        # Load get up motion data
        robot_suffix = "_2xc" if "2xc" in robot.name else "_2xm"

        get_up_prone_file_path = os.path.join(
            "motion", f"get_up_prone{robot_suffix}.lz4"
        )
        get_up_prone_left_file_path = os.path.join(
            "motion", f"get_up_prone_left{robot_suffix}.lz4"
        )
        get_up_prone_right_file_path = os.path.join(
            "motion", f"get_up_prone_right{robot_suffix}.lz4"
        )
        get_up_prone_both_file_path = os.path.join(
            "motion", f"get_up_prone_both{robot_suffix}.lz4"
        )
        get_up_prone_ref = joblib.load(get_up_prone_file_path)
        get_up_prone_left_ref = joblib.load(get_up_prone_left_file_path)
        get_up_prone_right_ref = joblib.load(get_up_prone_right_file_path)
        get_up_prone_both_ref = joblib.load(get_up_prone_both_file_path)

        motion_data = {
            "prone": get_up_prone_ref,
            "prone_left": get_up_prone_left_ref,
            "prone_right": get_up_prone_right_ref,
            "prone_both": get_up_prone_both_ref,
        }
