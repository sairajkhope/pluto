...
class ClimbUpBoxReference(MotionReference):
    def __init__(
        self, robot, dt: float, fixed_base: bool = False, playback_speed: float = 1.0
    ):
        super().__init__("climb_up_box", "keyframe", robot, dt, fixed_base)
        robot_suffix = "_2xc" if "2xc" in robot.name else "_2xm"
        motion_file_path = os.path.join("motion", f"climb_up_box{robot_suffix}.lz4")
